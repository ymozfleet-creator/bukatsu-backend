...
function openPlayerDetailModal(pid){
  ...
  var myRole = DB.currentUser?.role;
  if(myRole === 'team') {
    var myTeam = getMyTeam();
    if(myTeam && p.team !== myTeam.id) { toast('他チームの選手情報は閲覧できません','e'); return; }
  } else if(myRole === 'coach') {
    var myCoach = getMyCoach();
    var hasAccess = (DB.disclosures||[]).some(function(d){ return d.coachId===myCoach?.id && d.status==='active' && (d.playerIds||[]).includes(pid); });
    if(!hasAccess && myRole !== 'admin') { toast('この選手の情報開示が必要です','e'); return; }
  }
  const log=DB.trainingLog[p.id]||{};
