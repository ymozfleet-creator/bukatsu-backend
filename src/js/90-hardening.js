// 90 - セキュリティ強化: DevTools検知, コンテキストメニュー, 変数隠蔽
// ================================================================
...
// ================================================================
// ⑯ グローバルエラーハンドラ（スタックトレース漏洩防止）
// ================================================================
window.addEventListener('error', function(e) {
  // スタックトレースをユーザーに見せない
  if(e.error && typeof addAuditLog === 'function'){
    addAuditLog('js_error', (e.message||'').slice(0,100));
  }
});
window.addEventListener('unhandledrejection', function(e) {
  if(typeof addAuditLog === 'function'){
    addAuditLog('promise_error', String(e.reason||'').slice(0,100));
  }
  e.preventDefault(); // コンソールへの出力を抑制
});
