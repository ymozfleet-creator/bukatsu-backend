// 22-firebase-startup.js — Firebase, startup
...
function initSentry() {
  if(typeof Sentry !== 'undefined' && Sentry.init) {
    ...
    if(!dsn || dsn.includes('YOUR_KEY')) return; // DSN未設定時はスキップ
    try {
      Sentry.init({
        dsn: dsn,
        ...
        ignoreErrors: ['Network request failed','Failed to fetch','Load failed','ResizeObserver loop','Non-Error promise rejection','AbortError'],
